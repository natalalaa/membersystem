...
*/
		...
		$this->db->select("ff_persons.uid, email, active+0 AS active, date_format(ff_persons.created,'%d/%m %k:%i') AS created, CONCAT(firstname,' ',middlename, ' ',lastname) AS name, tel", false);
...
//        $this->javascript->output($js);
		...
		$this->db->select("division, pickupdate, date_format(`ff_itemdays`.`lastorder`,'%d/%m %k:%i') as lastorder, uid", FALSE);
...
AND ff_pickupdates.division = ff_items.division
	...
	function _saveexceldata($sheet, $division, $currentrow)
	{
		...
		$query = $this->db->query('SELECT firstname, middlename, lastname, email, tel, created FROM ff_persons' . $select . ' order by firstname');
		foreach ($query->result() as $row)
		{
	...
	function excel($division = 5) {
		...
		$query = $this->db->query('SELECT uid,firstname, middlename, lastname, email, tel, created FROM ff_persons' . $select . ' order by firstname');
		$currentrow = 1;
		foreach ($query->result() as $row)
		{
